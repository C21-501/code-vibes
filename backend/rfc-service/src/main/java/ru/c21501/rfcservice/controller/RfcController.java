...
import ru.c21501.rfcservice.service.*;
...
public class RfcController {
    ...
    public ResponseEntity<Page<RfcResponse>> getAllRfcs(
        ...
        Specification<Rfc> spec = RfcSpecification.hasStatus(status)
                .and(RfcSpecification.hasPriority(priority))
                .and(RfcSpecification.hasCreatedById(initiatorId))
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<RfcResponse> getRfcById(@PathVariable UUID id) {
    ...
    public ResponseEntity<RfcResponse> createRfc(@Valid @RequestBody CreateRfcRequest request) {
        ...
        // Найти создателя и заявителя
        User createdBy = userService.findById(request.getCreatedById())
                .orElseThrow(() -> new IllegalArgumentException("Создатель с ID " + request.getCreatedById() + " не найден"));
        User requester = userService.findById(request.getRequesterId())
                .orElseThrow(() -> new IllegalArgumentException("Заявитель с ID " + request.getRequesterId() + " не найден"));

        // Создать RFC entity
        Rfc rfc = rfcMapper.toEntity(request);
        rfc.setCreatedBy(createdBy);
        rfc.setRequester(requester);
    ...
    public ResponseEntity<RfcResponse> updateRfc(
        ...
        return rfcService.findById(id)
                .map(existingRfc -> {
                    ...
                    if (existingRfc.getStatus() == RfcStatus.IMPLEMENTED ||
    ...
    public ResponseEntity<List<StatusHistoryResponse>> getRfcStatusHistory(@PathVariable UUID id) {
    ...
    public ResponseEntity<RfcResponse> changeRfcStatus(
        ...
        try {
            ...
            // Если все исполнители подтвердили готовность, автоматически изменить статус на UNDER_REVIEW
            if (request.getNewStatus() == RfcStatus.APPROVED && rfcService.areAllExecutorsConfirmed(id)) {
                updatedRfc = rfcService.changeStatus(id, RfcStatus.UNDER_REVIEW, currentUser);
                log.info("All executors confirmed readiness, automatically changed RFC {} status to UNDER_REVIEW", id);
            }
    ...
    public ResponseEntity<RfcResponse> confirmExecutorReadiness(
        ...
        try {
            ...
            if (rfcService.areAllExecutorsConfirmed(id)) {
                // Автоматически изменить статус RFC на UNDER_REVIEW
                Rfc updatedRfc = rfcService.changeStatus(id, RfcStatus.UNDER_REVIEW, currentUser);
                log.info("All executors confirmed readiness, automatically changed RFC {} status to UNDER_REVIEW", id);
