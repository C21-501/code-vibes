...
public class DashboardServiceImpl implements DashboardService {
    ...
    public DashboardResponse getDashboardData() {
        ...
        stats.setDraft(rfcRepository.countByStatus(RfcStatus.DRAFT));
        stats.setReview(rfcRepository.countByStatus(RfcStatus.UNDER_REVIEW));
        stats.setApproved(rfcRepository.countByStatus(RfcStatus.APPROVED));
        stats.setImplemented(rfcRepository.countByStatus(RfcStatus.IMPLEMENTED));
        stats.setRejected(rfcRepository.countByStatus(RfcStatus.REJECTED) +
                rfcRepository.countByStatus(RfcStatus.CANCELLED));

        response.setStats(stats);

        // Последние RFC
        List<Rfc> recentRfcs = rfcRepository.findTop10ByOrderByCreatedAtDesc();
    ...
    private RecentRfcResponse mapToRecentRfcResponse(Rfc rfc) {
        ...
        response.setId(rfc.getId().toString());
        response.setTitle(rfc.getTitle());
        response.setStatus(rfc.getStatus());
        response.setPriority(rfc.getPriority());
        response.setCreatedDate(rfc.getCreatedAt());
        response.setAuthorName(rfc.getCreatedBy() != null ?
                rfc.getCreatedBy().getFirstName() + " " + rfc.getCreatedBy().getLastName() :
                "Неизвестно");
        return response;
    }
    ...
    private UpcomingDeadlineResponse mapToUpcomingDeadlineResponse(Rfc rfc) {
        ...
        response.setId(rfc.getId().toString());
        response.setTitle(rfc.getTitle());
        response.setDeadline(rfc.getPlannedStartDate());
        response.setStatus(rfc.getStatus());
        return response;
    }
}
